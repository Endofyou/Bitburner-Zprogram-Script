...
export async function main(ns) {
	...
	// Sets up the script window
	ns.clearLog();
	ns.disableLog("ALL");
	ns.tail();
	ZprogramIntro();

	// this block determines the best host server to run Zprogram's script chains (unless manually inputted).
	if (ns.args[1] == "best" || (ns.args[0] == null && ns.args[1] == null)) {
		...
		for (var i = 0; i < purchasedarray; i++) {
			var hostcandidate = ns.getServerMaxRam(purchasedservers[i]) -
				ns.getServerUsedRam(purchasedservers[i]);
	...
	if (ns.args[0] == "best" || (ns.args[0] == null && ns.args[1] == null)) {
		...
		for (var i = 0; i < arraylength; i++) {
			...
			if (bestserver <= percent * besthackchance * ns.getServerMaxMoney(serverarray[i]) *
...
export async function main(ns) {
	...
	while (true) {
		...
		do {
			var freeRam = ns.getServerMaxRam(host) - ns.getServerUsedRam(host),
				...
				seclvlRemoved = seclvl - minSeclvl,//security level to be removed
				weakThreads = seclvlRemoved / 0.05,//weaken threads to remove seclvlRemoved
				growthMultiplier = ns.getServerMaxMoney(target) / ns.getServerMoneyAvailable(target),
				growThreads = ns.growthAnalyze(target, growthMultiplier, 1),//grow threads
				extraWeakThreads = (growThreads * 0.004) / 0.05 + 2,//additional weaken threads from grow threads
				extraWeakGrowRam = (growThreads + extraWeakThreads) * 1.75,//grow (and consequential weaken) RAM usage
			...
			if (seclvl > minSeclvl) {
				...
				if (maxMoney > currentMoney) {
					if (Math.ceil(weakThreads * 1.75) + Math.ceil(extraWeakGrowRam) < freeRam) {
						...
						ns.exec("growonce.js", host, Math.floor(freeRam / 1.75 - weakThreads -
							(0.004 * (freeRam / 1.75 - weakThreads) / 0.05)) - 1, target);
						ns.exec("weakenonce.js", host, Math.floor(weakThreads +
							(0.004 * (freeRam / 1.75 - weakThreads) / 0.05)) + 1, target);
					} else {
				...
				if (Math.ceil(extraWeakGrowRam) < freeRam) {
					...
					ns.exec("growonce.js", host, Math.floor(
						growThreads * extraWeakGrowRatio) - 1, target);
					ns.exec("weakenonce.js", host, Math.floor(
						extraWeakThreads * extraWeakGrowRatio) + 1, target);
				}
			}
		...
		// Variables to be used in the following blocks.
		var freeRam = ns.getServerMaxRam(host) - ns.getServerUsedRam(host),
		...
		// this block calculates the optimal percentage of money to hack as well as important variables.
		for (var i = 1; i < 100; i++) {
			var percent = i / 100,
	...
	function CustomGAnalyzeHThread(variable, number) {
		...
		if (number == 0) {
			...
			return 1 - 1 / (10 ** (variable * Math.log10(1.0035) *
				ns.getHackingMultipliers(serverarray[i]).growth *
				serverGrowthPercentage * bitnode));
		}
	}
	...
	function CalcWtimeHchance(number) {
		...
		if (number == 0) {
			return ((reqhacklvl * minseclvl * 2.5 + 500) /
				(reqhacklvl * getseclvl * 2.5 + 500)) * hacktime * 4;
		} else if (number == 1) {
			var percenta = (100 - minseclvl) / 100,
				percentb = (100 - getseclvl) / 100,
				percentc = 1.75 * hacklvl,
				percentf = percenta * (percentc - reqhacklvl) / percentc,
				percentg = percentb * (percentc - reqhacklvl) / percentc,
				calchackchance = hackchance * percentf / percentg;
			if (calchackchance > 1) {
	...
	function ZprogramIntro() {
		return ns.print("Now running Zprogram by u/DryFacade"),
			ns.print(" "),
			ns.print('Please note: if the calculated time between each script is extremely brief, ',
