export async function main(ns) {
    ...
    if (ns.args[0] == "best" || (ns.args[0] == null && ns.args[1] == null)) {
        ...
        for (var i = 0; i < arraylength; i++) {
            ...
            if (
                bestserver <=
                    (percent * besthackchance * ns.getServerMaxMoney(serverarray[i]) * bestinstances) /
    ...
    if (ns.hasRootAccess(target) == true) {
        ...
        if (ns.getServerNumPortsRequired(target) - totalports <= 0) {
            ...
            ns.print(
                "Error: unable to gain root access.",
...
export async function main(ns) {
    ...
    var zfiles = ["growonce.js", "weakenonce.js", "zhack.js", "zgrow.js", "zweaken.js"];
    ...
    while (true) {
        ...
        do {
                ...
                if (maxMoney > currentMoney) {
                    if (Math.ceil(weakThreads * 1.75) + Math.ceil(extraWeakGrowRam) < freeRam) {
                        ...
                        ns.exec(
                            "growonce.js",
                            host,
                            Math.floor(freeRam / 1.75 - weakThreads - (0.004 * (freeRam / 1.75 - weakThreads)) / 0.05) -
                                1,
                            target
                        );
                        ns.exec(
                            "weakenonce.js",
                            host,
                            Math.floor(weakThreads + (0.004 * (freeRam / 1.75 - weakThreads)) / 0.05) + 1,
                            target
                        );
                    } else {
            ...
            if (seclvl > minSeclvl || maxMoney > currentMoney) {
                ns.print(
                    protocol,
                    " will take ",
                    Math.round(ns.getWeakenTime(target) + 1000) / 1000,
        ...
        if (instances <= 0) {
            ...
            for (var i = 1; i < 1 / 0; i++) {
                ...
                if (breaktime == true) {
                    ...
                    ns.print(
                        "There is not enough server RAM to run this script with target set as ",
                        target,
                        ". Now terminating Zprogram..."
                    );
                    ns.exit();
                }
            }
        ...
        ns.print(
            ...
            Math.round(timeBetweenInstances * instances2 * 3) / 1000,
            " seconds."
        );
        if (percent <= 0.01) {
    ...
    function CustomHackAnalyze(percent) {
        return (
            (ns.getServerMaxMoney(serverarray[i]) * percent) /
            (ns.getServerMaxMoney(serverarray[i]) *
                ns.hackAnalyze(serverarray[i]) *
    ...
    function CustomGAnalyzeHThread(variable, number) {
        ...
        var bitnode =
            Math.log(growthp) /
            (Math.log(ajdGrowthRatebit) * ns.getHackingMultipliers(serverarray[i]).growth * serverGrowthPercentage) /
    ...
    function CalcWtimeHchance(number) {
        ...
        if (number == 0) {
            return ((reqhacklvl * minseclvl * 2.5 + 500) / (reqhacklvl * getseclvl * 2.5 + 500)) * hacktime * 4;
        } else if (number == 1) {
